...
| **IOT\_BLUETOOTH\_A2DP\_SINK** | Adds ability to receive and play A2DP audio from an external device to the IoT device. |
| **IOT\_BLUETOOTH\_A2DP\_SOURCE** | Adds ability to play audio to an external device.  This was implicitly included in Windows 10 IoT, version 1803, but must be explicitly specified for newer versions |
| **IOT\_BLUETOOTH\_HFP\_AUDIOGATEWAY** | Adds Bluetooth Handsfree-Profile AudioGateway role.|
...
| ~~IOT\_CORTANA~~                | (deprecated).  This is no longer available in Windows 10 IoT. Refer to [The Cortana Dev Center](http://www.aka.ms/cortanadevices)~~Cortana may be included in IoT with an licensing agreement. Adds Cortana feature. Requires **IOT\_APPLICATIONS** feature. This is new in Windows 10, version 1703.       ~~|
| ~~IOT\_CORTANA\_OBSCURELAUNCH~~ | (deprecated).  This is no longer available in Windows 10 IoT. Refer to [The Cortana Dev Center](http://www.aka.ms/cortanadevices) ~~Cortana may be embedded into Windows IoT with a licensing agreement. Enables running Cortana application on boot. This add-on causes Cortana to run in the background resulting in better response time for Cortana. This is new in Windows 10, version 1703.~~ |
